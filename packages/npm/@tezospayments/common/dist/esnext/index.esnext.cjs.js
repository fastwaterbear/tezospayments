...
const redditLinkInfoProvider = link => socialMediaLinkInfoProvider(link, 'https://www.reddit.com/', exports.IconId.Reddit);
...
class PaymentParserBase {
    ...
    validateRawPayment(rawPayment) {
        const rawPaymentFieldNames = Object.getOwnPropertyNames(rawPayment);
...
const validateAmount = (amount, errors) => {
    if (!bignumber_js.BigNumber.isBigNumber(amount) || amount.isNaN() || !amount.isFinite())
...
class DonationValidator extends PaymentValidatorBase {
    ...
    validationMethods = [
        donation => donation.type !== exports.PaymentType.Donation ? [DonationValidator.errors.invalidType] : undefined,
...
class Payment extends StateModel {
    static defaultParser = new PaymentParser();
...
class Donation extends StateModel {
    static defaultParser = new DonationParser();
