...
class Base64PaymentUrlFactory extends PaymentUrlFactory {
    ...
    createDonationUrlInternal(donation, network) {
        ...
        try {
            const url = new common.native.URL(`${donation.targetAddress}/donation`, this.baseUrl);
    ...
    createUrl(baseUrl, serializedPaymentOrDonationBase64, network) {
        if (serializedPaymentOrDonationBase64 !== '')
            baseUrl.hash = common.getEncodedPaymentUrlType(this.urlType) + serializedPaymentOrDonationBase64;
...
class ApiSecretKeySigner extends TezosPaymentsSigner {
    apiSecretKey;
    paymentSignPayloadEncoder = new common.PaymentSignPayloadEncoder();
    inMemorySigner;
...
class WalletSigner extends TezosPaymentsSigner {
    ...
    async sign(payment) {
        ...
        const signingPromises = signPayload.clientSignPayload
            ? [contractSigningPromise, this.walletSignCallback(signPayload.clientSignPayload)]
            : [contractSigningPromise];
        const signatures = await Promise.all(signingPromises);
        return {
...
class TezosPaymentsOptionsValidator {
    ...
    validateNetwork(network) {
        ...
        if (typeof network.name !== 'string' || !common.networkNameRegExp.test(network.name))
            return [TezosPaymentsOptionsValidator.errors.invalidNetworkName];
        if (network.id && (typeof network.id !== 'string' || !common.networkIdRegExp.test(network.id)))
...
class TezosPayments {
    ...
    optionsValidator = new TezosPaymentsOptionsValidator();
